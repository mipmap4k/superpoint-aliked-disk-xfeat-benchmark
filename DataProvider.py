...
class DataProvider:
    def __init__(self, dataset_path, camera_specs=None):
        ...
        self.camera_specs = camera_specs if camera_specs else {
    ...
    def _get_satellite_patch(self, map_path, lat, lon, altitude, yaw, target_shape):
        ...
        # Проверка валидности параметров для предотвращения NaN/Inf
        if altitude <= 0 or not np.isfinite(altitude):
            altitude = 100.0  # Значение по умолчанию
        ...
        with rasterio.open(map_path) as src:
            ...
            if not np.isfinite(map_x) or not np.isfinite(map_y):
                raise ValueError(f"Transformation resulted in NaN/Inf: map_x={map_x}, map_y={map_y}")
            
            # Получаем пиксельные координаты центра на исходной карте
            py, px = src.index(map_x, map_y)
            
            # 2. Расчет масштаба
            drone_gsd = self._calculate_gsd(altitude, target_w) # метры/пиксель на фото дрона
            map_transform = src.transform
            map_res_x = map_transform[0] # метры/пиксель на карте (предполагаем квадратный пиксель)
            ...
            window = Window(px - crop_size_map_px // 2, py - crop_size_map_px // 2, 
                            crop_size_map_px, crop_size_map_px)
        ...
        rotated_map = cv2.warpAffine(map_crop, M, (map_crop.shape[1], map_crop.shape[0]))
        ...
        final_crop = rotated_map[start_y:start_y+final_height_map_px, start_x:start_x+final_width_map_px]
        
        if final_crop.size == 0:
            return np.zeros((target_h, target_w, 3), dtype=np.uint8)

        # 6. Финальный ресайз до размера изображения с дрона
        satellite_image = cv2.resize(final_crop, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
    ...
    def generator(self, shuffle=False):
        ...
        for idx in indices:
            ...
            try:
                ...
                # Пропускаем кадр при ошибке обработки карты
                continue
